...
function LoadFile(lat, lng){
    polygon_coordinates[ncities] = new google.maps.LatLng(lat, lng);
...
function initialize() {
    ...
    var myOptions = {
	zoom: 11,
	center: new google.maps.LatLng(-3.824164831099026, -38.520054681396484),
...
function Update(){
    ...
    google.maps.event.addListener(map, 'dblclick', function(event) {
		var coodinat = document.getElementById('coodinatesn');
		coodinat.value = event.latLng.lat()+', '+ event.latLng.lng();
		var marker = new google.maps.Marker({
			map: map, 
			position: event.latLng,
			title: coodinat.value
		});
	});
}
...
function savePolygon(){
	...
	for(var i=0; i< polygon_coordinates.length;i++){
		csvContent += polygon_coordinates[i].lat()+', '+polygon_coordinates[i].lng()+"\n";
	...
	link.setAttribute("download", retVal+".csv");
...
function CheckCoodin(){
	
	var cood = document.getElementById('coodinatesn');
	var datas = cood.value.split(',');
	var coodins = new google.maps.LatLng(datas[0], datas[1]);
	if(PointInsidePolygon(GetPixelCoordinates(coodins))){
		alert('Dentro!');
	}else{
		alert('Fora!');
	}
